...
path solve_left(Maze& m, int rows, int cols)
{
    ...
    while(next_row < m.columns()-1 || next_col < m.rows()-1) {
...
path solve_dfs(Maze& m, int rows, int cols)
{
    ...
    // Calculate our path points and push them onto the list
    while(current_point.first < m.columns()-1 || current_point.second < m.rows()-1) {
        ...
        if      (current_nodeinfo.exits[DOWN]  == 1) {
            ...
            // If there are no exits then either we are at the end or we need to backtrack
            if (next_point.first == m.columns()-1 && next_point.second == m.rows()-1) {
...
//                point up_point = make_pair(current_point.first  , current_point.second-1);
...
//                point left_point = make_pair(current_point.first-1, current_point.second  );
...
//                point down_point = make_pair(current_point.first  , current_point.second+1);
...
//                point right_point = make_pair(current_point.first+1, current_point.second  );
